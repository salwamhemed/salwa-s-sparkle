...
import { Brain, Eye, BarChart3, Code2, Globe, Wrench, Heart, Sparkles } from "lucide-react";
...
const AboutSection = () => (
  <section id="about" className="py-24 relative overflow-hidden">
    {/* Decorative blobs */}
    <div className="absolute top-0 right-0 w-64 h-64 rounded-full bg-primary/5 blur-3xl animate-float-slow" />
    <div className="absolute bottom-20 left-0 w-48 h-48 rounded-full bg-accent/10 blur-3xl animate-float" />
    ...
    <div className="container max-w-5xl relative">
      ...
      >
        <motion.span
          ...
          className="inline-flex items-center gap-2 text-xs font-medium tracking-widest uppercase text-primary mb-3"
        >
          <Heart size={14} className="fill-primary" /> About Me
        </motion.span>
      ...
      <motion.div
        ...
        className="glass-card p-8 mb-14 relative group"
      >
        <motion.div
      ...
      >
        {skillCards.map((card) => (
          <motion.div
            ...
            whileHover={{ y: -6 }}
            className="glass-card p-6 group cursor-default transition-shadow duration-300 hover:shadow-[var(--shadow-soft)]"
          >
            <motion.div
              className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-colors duration-300"
